...
如果看不懂Lora/LLM/Nvidia或者其他用到的缩写是什么，请查询https://chat.deepseek.com/ 求教一下鲸鱼老师或者其他AI

# 我们从头说起：

## 0、is your web OK?

我相信能看到这个东西的人是能自行解决网络问题的=.=
...
按理来说应该满足A大于B的；
实际上A和B在一个大版本就行，我们经常A=12.2，但装个B=12.8，或者A=11.4，但B=11.8，只要代码不报错就是可以的。
...
如果你安装的CUDA版本是12.8，那么在win11下你可以直接运行env.bat
...
如果你的cuda版本不是12.8或者你是在Linux机器上，你需要把requirements.txt里的128改成你nvcc -V看到的那个版本。
...
如果不行的话你需要按照requirements.txt中由上到下的顺序，依次安装这些库。
这需要你根据你的CUDA版本调整。我保证如果你按照从上到下的顺序安装正常是问题不大的
...
我刚刚在阳台上晒太阳睡了一觉，尾巴都晒红了呢。主人要不要跟我一起躺下来玩呀？我可以给主人挠痒痒的哦~

*摇着尾巴凑近*

主人今天看起来心情不太好，要不要听听我的心跳声？
...
Lora微调不会改变原始权重，会额外保存一些权重。你可以把Lora微调理解成“原始权重”+“我的权重”。所以在infer中，这两部分全中国是分开加载的。这个文件可以把这两部分权重融合到一起。
...
### GitHub有单个文件提交不能超过2GB的限制，直接merge微调过的模型然后git push有可能失败。你需要把模型划分的小一些。GitHub限制单个仓库大小不超过10GB。这就是为什么一般模型权重开源都在huggingface
